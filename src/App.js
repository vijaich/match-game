...
const initialMatchGameDetails = {
  tabsListItems: tabsList,
  randomImgIndex: 0,
  selectedTab: tabsList[0].tabId,
  score: 0,
  isGameOver: false,
  counter: 60,
  selectedTabImages: imagesList.filter(
    each => each.category === tabsList[0].tabId,
  ),
}

class App extends Component {
  state = initialMatchGameDetails

  componentDidMount = () => {
    this.timerInterval = setInterval(this.onDecreaseNum, 1000)
  }
  ...
  onDecreaseNum = () => {
    const {counter, isGameOver} = this.state
  ...
  onClickTab = tabId => {
    const newSelectedTabImages = imagesList.filter(
      each => each.category === tabId,
    )
    this.setState({selectedTabImages: newSelectedTabImages})
  }
  ...
  onselectImg = id => {
    ...
    this.setState({randomImgIndex: randomNum})
  ...
  onClickReplay = () => {
    this.setState(initialMatchGameDetails)
  }
  ...
  render() {
    ...
    const counterNum = counter > 9 ? counter : `0${counter}`
    const randomImgUrl = imagesList[randomImgIndex].imageUrl
    const gameContainer = isGameOver ? 'containerHide' : ''
    ...
    return (
      <div className="match-game-bg-container">
        <div className="nav-container">
          ...
          />
          <div className="top-nav-timer-score-container">
            ...
            <img
              ...
              className="timer-img"
            />
            <p className="timer">{counterNum} Sec</p>
          </div>
        </div>
        <div className="bottom-match-game-container">
          {isGameOver ? (
            <div className="score-card">
              <img
                ...
                className="trophy"
              />
              <p className="score-txt-head">Your Score</p>
              ...
              >
                ...
                />
                <p className="play-again-txt">PLAY AGAIN</p>
              </button>
            </div>
          ) : (
            ''
          )}
          <div className={`card-container ${gameContainer}`}>
            <img src={randomImgUrl} className="random-image" alt="fff" />
            <ul className="tab-item-container">
              {tabsListItems.map(each => (
                <TabItems
                  key={each.tabId}
                  eachTab={each}
                  onTabBtnClick={this.onClickTab}
                  ImageListItems
                />
              ))}
            </ul>
            ...
            <ul className="images-container">
              {selectedTabImages.map(eachImage => (
                <ImageListItems
                  key={eachImage.id}
                  eachList={eachImage}
                  imgSelected={this.onselectImg}
                />
              ))}
            </ul>
          </div>
